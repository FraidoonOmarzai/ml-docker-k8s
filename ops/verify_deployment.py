...
def c(text, col): return f"{C[col]}{text}{C['reset']}"
def section(t):
    print(f"\n{c('─'*58, 'cyan')}")
    print(c(f"  {t}", "bold"))
    print(c('─'*58, 'cyan'))
...
def check(name, ok, detail="", warn_only=False):
    ...
    if ok:
        ...
        if detail: print(f"       {c(detail, 'yellow')}")
        warnings += 1
    else:
        print(f"  {c('❌', 'red')} {name}")
        if detail: print(f"       {c(detail, 'red')}")
        failed += 1
...
def check_kubectl(namespace, deployment):
    ...
    check(f"Ready replicas ≥ 1 (got {ready})", ready >= 1,
          "No ready replicas found")
    ...
    check(f"No unavailable replicas (got {unavail})", unavail == 0,
          f"{unavail} unavailable replicas", warn_only=(unavail > 0))

    ok, out, _ = kubectl(f"get hpa -n {namespace}")
    check("HPA exists", ok and "iris-ml-api" in out, warn_only=True)

    ok, out, _ = kubectl(f"get svc iris-ml-api -n {namespace}")
...
def check_inference(base_url):
    ...
    # Batch
    status, body = http_post(f"{base_url}/predict/batch",
...
def main():
    ...
    print(c("\n🚀 Iris ML API — Live Deployment Verification", "bold"))
