...
def print_report(duration_actual: float, num_workers: int, target_rps: int):
    ...
    print("     kubectl get hpa -n ml-system -w")
    print()


# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Load test the Iris ML API")
    ...
    print("       kubectl get hpa -n ml-system -w")
    print("       kubectl get pods -n ml-system -w\n")
    ...
    for i in range(args.workers):
        t = threading.Thread(target=worker,
                             args=(args.url, i, rps_per_worker),
                             daemon=True)
        t.start()
        threads.append(t)
    ...
    prog = threading.Thread(target=print_progress,
                            args=(args.duration,), daemon=True)
    prog.start()
